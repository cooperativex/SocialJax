...
def make_train(config, env):
    ...
    def unbatchify(x: jnp.ndarray):
        ...
        return {str(agent): x[i] for i, agent in enumerate(env.agents)}
    ...
    def train(rng):
        ...
        def create_agent(rng):
            # SocialJax returns list of spaces
        ...
        # Use string keys to match CTRolloutManager format
        _actions = {
            str(agent): wrapped_env.batch_sample(_rng, agent) for agent in env.agents
        }
        _obs, _, _rewards, _dones, _infos = wrapped_env.batch_step(
            _rng, _env_state, _actions
        )
        _avail_actions = wrapped_env.get_valid_actions(_env_state)
        _timestep = Timestep(
            obs=_obs,
            actions=_actions,
        ...
        def _update_step(runner_state, unused):
            ...
            def _step_env(carry, _):
                ...
                # SocialJax environments may not have shaped_reward
...
@hydra.main(version_base=None, config_path="./config", config_name="vdn_cnn_harvest_partnership")
def main(config):
