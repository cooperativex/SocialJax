        ...
        def _update_step(runner_state, unused):
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(states, batch_info):
                    ...
                    # --- reward model loss ---
                    def reward_loss_fn(params, num_agents=config['ENV_KWARGS']['num_agents']):
                        action_for_reward = jnp.reshape(traj_batch.action, (-1, num_agents))
                        obs_for_reward = jnp.reshape(traj_batch.obs, (-1, num_agents, *(traj_batch.obs.shape[1:])))
                        pred_reward = reward_state.apply_fn(params, obs_for_reward, action_for_reward)
                        pred_reward = jnp.reshape(pred_reward, (-1,))
                        loss = jnp.mean((pred_reward - traj_batch.reward) ** 2)
                        return loss
                    reward_grad_fn = jax.value_and_grad(reward_loss_fn)
                    reward_loss, reward_grads = reward_grad_fn(reward_state.params)
                    reward_state = reward_state.apply_gradients(grads=reward_grads)
                    return (train_state, reward_state), (total_loss,reward_loss)

                train_state, reward_state, traj_batch, advantages, targets, rng = update_state
                rng, _rng = jax.random.split(rng)
                ...
                (train_state, reward_state), loss_info = jax.lax.scan(
                    _update_minbatch, (train_state, reward_state), minibatches
                )
                update_state = (train_state, reward_state, traj_batch, advantages, targets, rng)
            ...
            metric["reward_loss"] = loss_info[1].mean()
            metric["total_loss"] = loss_info[0][0].mean()
