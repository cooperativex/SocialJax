...
from socialjax.algorithms.cf.counterfactual import (
    ...
    generate_counterfactual_rewards_topk,
    compute_collective_cf_reward,
    compute_collective_cf_reward_topk,
    compute_actual_collective_reward,
    get_counterfactual_analysis,
    get_counterfactual_analysis_fast,
)
...
class CFConfig:
    ...
    # Speed optimization
    top_k_actions: int = 0  # 0 = use all actions, >0 = only top-k actions (2-4x faster)
    use_mixed_precision: bool = False  # Enable bfloat16 for faster computation
...
class CFTrainer:
    ...
    def _compile_functions(self):
        ...
        # Check if using optimized counterfactual computation
        use_top_k = self.config.top_k_actions > 0
        top_k = self.config.top_k_actions if use_top_k else self.action_dim
        ...
        def _compute_shaped_rewards_batch(
            ...
            def _compute_for_step(step_data):
                ...
                if use_top_k:
                    # Optimized: only compute for top-k actions
                    ...
                    collective_cf = compute_collective_cf_reward_topk(cf_rewards, exclude_self=True)
                else:
                    # Original: compute for all actions
