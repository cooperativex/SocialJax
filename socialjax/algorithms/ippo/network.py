"""Neural network architectures for IPPO algorithm.

This module provides the Actor-Critic network architecture used by the
Independent PPO algorithm for multi-agent reinforcement learning.
"""
...
class IPPOCNN(nn.Module):
    """Convolutional Neural Network for visual feature extraction in IPPO.
...
@register_network("ippo_actor_critic")
class IPPOActorCritic(nn.Module):
    """Combined Actor-Critic network for IPPO algorithm.
    ...
    def __call__(self, x):
        ...
        embedding = IPPOCNN(self.activation)(x)
