...
from socialjax.algorithms.mappo.network import MAPPOActor, MAPPOCritic, MAPPOActorCritic
...
class MAPPOAlgorithm(BaseAlgorithm):
    ...
    def _build_network(self):
        """Build and return the combined Actor-Critic network for the trainer.

        Also stores separate actor and critic networks internally for the
        algorithm's own methods.

        Returns:
            Combined MAPPOActorCritic network for the unified trainer
        """
        action_dim = self.action_space.n if hasattr(self.action_space, 'n') else self.action_space
        activation = self.config.get("ACTIVATION", "relu")
        hidden_size = self.config.get("HIDDEN_SIZE", 64)

        # Store separate networks for internal algorithm use
        self._actor_network = MAPPOActor(
            action_dim=action_dim,
            activation=activation,
            hidden_size=hidden_size,
        )

        self._critic_network = MAPPOCritic(
            activation=activation,
            hidden_size=hidden_size,
        )

        # Return combined network for the unified trainer
    ...
    def _build_optimizer(self):
        """Build and return a single optimizer for the combined network.

        Returns:
            Optax optimizer for the combined network
        """
    ...
    def compute_value(
        ...
        """
        # Add batch dimension if needed (network expects batched input)
        # World state shape: (*obs_shape[:-1], obs_shape[-1] * num_agents)
        world_state_shape = (*self.observation_space.shape[:-1],
                            self.observation_space.shape[-1] * self.num_agents)
        single_state = world_state.ndim == len(world_state_shape)
        if single_state:
            world_state = world_state[jnp.newaxis, ...]

        critic_network = self.network[1]
        value = critic_network.apply(state.critic_params, world_state)
    ...
    def save(self, path: str) -> None:
        ...
        # Access state from self._state if available (set by trainer)
