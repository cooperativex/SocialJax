...
Examples:
  ...
  python scripts/train.py --algorithm ippo --env coin_game --wandb-project myproject
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of parallel environments (default: from config, typically 256)",
    )
    parser.add_argument(
        "--num-steps",
        type=int,
        default=None,
        help="Steps per rollout (default: from config, typically 1000)",
    )
    ...
    parser.add_argument(
        "--num-minibatches",
        type=int,
        default=32,  # Use 32 minibatches for MAPPO
    )
    parser.add_argument(
        "--update-epochs",
        type=int,
        default=4,
        help="Number of PPO update epochs (overrides config)",
    )
    parser.add_argument(
        "--no-anneal-lr",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Resume from checkpoint directory (e.g., checkpoints/ippo_coin_game/ippo_final)",
    )
...
def load_config(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if args.config and os.path.exists(args.config):
        ...
        from pathlib import Path
        config = config_manager.load_from_file(Path(args.config))
    else:
    ...
    if args.num_minibatches is not None:
        overrides.setdefault("algorithm", {}).setdefault("training", {})["num_minibatches"] = args.num_minibatches
    if args.update_epochs is not None:
        overrides.setdefault("algorithm", {}).setdefault("training", {})["update_epochs"] = args.update_epochs
    if args.no_anneal_lr:
        overrides.setdefault("algorithm", {}).setdefault("training", {})["anneal_lr"] = False
...
def print_training_info(args: argparse.Namespace, config: Dict[str, Any]):
    ...
    training_cfg = config.get("algorithm", {}).get("training", {})
    print(f"Num envs:      {training_cfg.get('num_envs', args.num_envs)}")
    print(f"Num steps:     {training_cfg.get('num_steps', args.num_steps)}")
...
def main():
    ...
    if args.resume:
        ...
        try:
            checkpoint_info = trainer.load(args.resume)
            start_timestep = checkpoint_info.get("timestep", 0)
            if args.verbose >= 1:
    ...
    try:
        ...
        if not _interrupted:
            ...
            trainer.save(final_path, timestep=state.timestep, update_step=state.update_step)
        ...
        if _interrupted:
            ...
            timesteps = trainer._state.timestep if hasattr(trainer, '_state') else 0
            update_steps = trainer._state.update_step if hasattr(trainer, '_state') else 0
            print(f"Saving checkpoint to: {interrupt_path}")
            trainer.save(interrupt_path, timestep=timesteps, update_step=update_steps)

            print("\n" + "=" * 60)
            print("Training Interrupted")
            print("=" * 60)
            print(f"Timesteps completed: {timesteps:,}" if isinstance(timesteps, int) else f"Timesteps completed: {timesteps}")
            print(f"Elapsed time:        {format_time(elapsed)}")
