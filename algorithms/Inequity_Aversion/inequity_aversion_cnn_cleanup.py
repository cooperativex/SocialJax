...
def single_run(config):
    ...
    wandb.init(
        ...
        name=f'inequity_aversion_cnn_cleanup',
        group=f'cleanup',
    )
...
def tune(default_config):
    ...
    sweep_config = {
        ...
        "parameters": {
            ...
            "ENV_KWARGS.inequity_aversion_alpha": {"values": [0.0, 0.1, 1, 10]},
            "ENV_KWARGS.inequity_aversion_beta": {"values": [0.0, 0.01, 0.1, 1]},
...
@hydra.main(version_base=None, config_path="config", config_name="inequity_aversion_cnn_cleanup")
def main(config):
    if config["TUNE"]:
        tune(config)
    else:
