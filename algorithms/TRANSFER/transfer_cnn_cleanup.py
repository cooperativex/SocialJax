...
def make_train(config):
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            metric["waste_cleared"] = metric.get("waste_cleared", 0) * config["ENV_KWARGS"]["num_inner_steps"]
...
@hydra.main(version_base=None, config_path="config", config_name="transfer_cnn_cleanup")
def main(config):
    if config["TUNE"]:
        tune(config)
    else:
