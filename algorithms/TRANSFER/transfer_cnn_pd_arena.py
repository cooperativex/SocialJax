...
def make_train(config):
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            metric["pd_actions"] = metric.get("pd_actions", 0) * config["ENV_KWARGS"]["num_inner_steps"]
...
@hydra.main(version_base=None, config_path="config", config_name="transfer_cnn_pd_arena")
def main(config):
    if config["TUNE"]:
        tune(config)
    else:
