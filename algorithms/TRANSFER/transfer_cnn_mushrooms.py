...
def calculate_s_interest_schedule(n_agents=5):
...
def make_train(config):
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            # Clean up specific metrics for cleanup environment
            metric["mushrooms_collected"] = metric.get("mushrooms_collected", 0) * config["ENV_KWARGS"]["num_inner_steps"]
...
@hydra.main(version_base=None, config_path="config", config_name="transfer_cnn_mushrooms")
def main(config):
    if config["TUNE"]:
        tune(config)
    else:
