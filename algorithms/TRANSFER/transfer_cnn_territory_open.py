...
def calculate_s_interest_schedule(n_agents=7):
...
def make_train(config):
    ...
    if config["ENV_KWARGS"].get("interest", False):
        ...
        config["ENV_KWARGS"]["s_interest_change_every"] = int(config["ENV_KWARGS"].get("s_interest_change_every", 1000000))
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                info = jax.tree_map(reshape_info_values, info)
            ...
            # Clean up specific metrics for territory_open environment
            metric["territory_captured"] = metric.get("territory_captured", 0) * config["ENV_KWARGS"]["num_inner_steps"]
...
def single_run(config):
    ...
    wandb.init(
        ...
        name=f'transfer_cnn_territory_open',
...
@hydra.main(version_base=None, config_path="config", config_name="transfer_cnn_territory_open")
def main(config):
    if config["TUNE"]:
        tune(config)
    else:
